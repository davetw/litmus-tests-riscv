...
inline static int final_cond(int _out_0_x1,int _out_1_x1,int _x,int _y) {
  switch (_out_0_x1) {
  case 1:
    switch (_out_1_x1) {
    case 1:
...
static const int out_0_x1_f = 0 ;
static const int out_0_x3_f = 1 ;
static const int out_1_x1_f = 2 ;
static const int out_1_x3_f = 3 ;
...
static void do_dump_outcome(FILE *fhist, intmax_t *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>0:x1=%d; 0:x3=%d; 1:x1=%d; 1:x3=%d; [x]=%d; [y]=%d;\n",c,show ? '*' : ':',(int)o[out_0_x1_f],(int)o[out_0_x3_f],(int)o[out_1_x1_f],(int)o[out_1_x3_f],(int)o[x_f],(int)o[y_f]);
}
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("ISA-2+2W-SUCCESS, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("ISA-2+2W-SUCCESS, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"ISA-2+2W-SUCCESS");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"sw %[x4],0(%[x9])\n"
"#END _litmus_P0\n"
:[x3] "=&r" (_a->out_0_x3[_i]),[x1] "=&r" (_a->out_0_x1[_i]),[x4] "=&r" (trashed_x4),[x2] "=&r" (trashed_x2)
...
"sw %[x4],0(%[x8])\n"
"#END _litmus_P1\n"
:[x3] "=&r" (_a->out_1_x3[_i]),[x1] "=&r" (_a->out_1_x1[_i]),[x4] "=&r" (trashed_x4),[x2] "=&r" (trashed_x2)
:[x8] "r" (&_a->x[_i]),[x9] "r" (&_a->y[_i])
:"cc","memory"
);
    }
...
static void init(ctx_t *_a) {
  ...
  _a->seed = rand();
  _a->out_0_x1 = malloc_check(size_of_test*sizeof(*(_a->out_0_x1)));
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      int _out_0_x1_i = ctx.out_0_x1[_i];
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) fatal("ISA-2+2W-SUCCESS, global x unstabilized") ;
      }
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_y_i != ctx.cpy_y[_p][_i]) fatal("ISA-2+2W-SUCCESS, global y unstabilized") ;
      }
      cond = final_ok(final_cond(_out_0_x1_i,_out_1_x1_i,_x_i,_y_i));

      if (_out_0_x1_i == 1 && _out_0_x3_i == 0 && _out_1_x1_i == 0 &&
          _out_1_x3_i == 0 && _x_i == 1 && _y_i == 1) {
        // For debugging
        printf("Found invalid execution\n");
        exit(-1);
      }
...
#else
#include "ISA-2+2W-SUCCESS.h"
#endif

static void prelude(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for /home/davew/work/litmus-tests-riscv/tests/non-mixed-size/HAND/ISA-2+2W-SUCCESS.litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test ISA-2+2W-SUCCESS Allowed\n");
  ...
  fprintf(out,"Condition %s is %svalidated\n","exists (0:x1=1 /\\ 1:x1=1 /\\ [x]=3 /\\ [y]=3)",cond ? "" : "NOT ");
  fprintf(out,"Hash=14b117d7771d5cb04241cad4285b25a4\n");
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation ISA-2+2W-SUCCESS %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time ISA-2+2W-SUCCESS %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "ISA-2+2W-SUCCESS: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("ISA-2+2W-SUCCESS, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int ISA_2D_2_2B_2W_2D_SUCCESS(int argc, char **argv, FILE *out) {
