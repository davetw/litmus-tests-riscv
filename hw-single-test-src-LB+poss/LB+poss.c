...
static void *monitor_thread(void *arg) {
    monitor_arg_t *m_arg = (monitor_arg_t *)arg;
    ctx_t *ctx = m_arg->ctx;
    int size = m_arg->size_of_test;
    int current_idx = size - 1;  // Start from the end
    
    while (!*(m_arg->should_stop)) {
        // Look for first non-MAGIC value
        while (current_idx >= 0) {
            int _out_0_x5_i = ctx->out_0_x5[current_idx];
            int _out_1_x5_i = ctx->out_1_x5[current_idx];
            int _x_i = ctx->x[current_idx];

            // Skip if values haven't been modified yet
            if (_out_0_x5_i == MAGIC_VALUE || 
                _out_1_x5_i == MAGIC_VALUE || 
                _x_i == MAGIC_VALUE) {
                current_idx--;
                continue;
            }
            ...
            if ((_out_0_x5_i == 0 && _out_1_x5_i == 1 && _x_i == 1) ||
                ...
                printf("Monitor found interesting pattern at iteration %d:\n", current_idx);
                printf("  Values: out_0_x5=%d, out_1_x5=%d, x=%d\n", 
                       _out_0_x5_i, _out_1_x5_i, _x_i);
                printf("  Addresses: &out_0_x5=%p, &out_1_x5=%p, &x=%p\n",
                       (void*)&ctx->out_0_x5[current_idx],
                       (void*)&ctx->out_1_x5[current_idx],
                       (void*)&ctx->x[current_idx]);
                printf("----------------------------------------\n");
            }
            current_idx--;
        }
        
        // Reset index if we've reached the beginning
        if (current_idx < 0) {
            current_idx = size - 1;
        }
...
static void reinit(ctx_t *_a) {
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    _a->x[_i] = MAGIC_VALUE;
    _a->out_0_x5[_i] = MAGIC_VALUE;
    _a->out_1_x5[_i] = MAGIC_VALUE;
    _a->barrier[_i] = 0;
  }
}
...
static void run(cmd_t *cmd, cpus_t *def_all_cpus, FILE *out) {
  ...
  volatile int monitor_should_stop = 0;

  // Setup monitor thread arguments
  monitor_arg_t monitor_arg = {
    .ctx = &ctx,
    .size_of_test = prm.size_of_test,
    .should_stop = &monitor_should_stop
  };

  // Launch monitor thread
  launch(&monitor_th, monitor_thread, &monitor_arg);
  ...
  // Stop and join monitor thread
  monitor_should_stop = 1;
  join(&monitor_th);
