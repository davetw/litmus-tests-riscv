...
inline static int final_cond(int _out_1_x5,int _out_1_x10,int _y) {
  switch (_out_1_x5) {
  case 1:
    switch (_out_1_x10) {
    case 0:
...
static const int out_1_x10_f = 1 ;
...
static void do_dump_outcome(FILE *fhist, intmax_t *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>1:x5=%d; 1:x10=%d; [y]=%d;\n",c,show ? '*' : ':',(int)o[out_1_x5_f],(int)o[out_1_x10_f],(int)o[y_f]);
}
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("MP+fence.rw.rw+amoswap-rfi-addr, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("MP+fence.rw.rw+amoswap-rfi-addr, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"MP+fence.rw.rw+amoswap-rfi-addr");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"sw %[x7],0(%[x8])\n"
"#END _litmus_P0\n"
:[x7] "=&r" (trashed_x7),[x5] "=&r" (trashed_x5)
...
"lw %[x10],0(%[x9])\n"
"#END _litmus_P1\n"
:[x10] "=&r" (_a->out_1_x10[_i]),[x5] "=&r" (_a->out_1_x5[_i]),[x9] "=&r" (trashed_x9),[x7] "=&r" (trashed_x7),[x4] "=&r" (trashed_x4),[x1] "=&r" (trashed_x1)
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      if (_out_1_x10_i == 1 && _out_1_x5_i == 1 && _y_i == 1) {
        // For debugging
...
#else
#include "MP+fence.rw.rw+amoswap-rfi-addr.h"
#endif

static void prelude(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for /home/davew/work/litmus-tests-riscv/tests/non-mixed-size/HAND/MP+fence.rw.rw+amoswap-rfi-addr.litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test MP+fence.rw.rw+amoswap-rfi-addr Allowed\n");
  ...
  fprintf(out,"Condition %s is %svalidated\n","exists ([y]=2 /\\ 1:x5=1 /\\ 1:x10=0)",cond ? "" : "NOT ");
  fprintf(out,"Hash=850d037e7d3a7cfd4729bdc923fbb91f\n");
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation MP+fence.rw.rw+amoswap-rfi-addr %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time MP+fence.rw.rw+amoswap-rfi-addr %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "MP+fence.rw.rw+amoswap-rfi-addr: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("MP+fence.rw.rw+amoswap-rfi-addr, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int MP_2B_fence_2E_rw_2E_rw_2B_amoswap_2D_rfi_2D_addr(int argc, char **argv, FILE *out) {
